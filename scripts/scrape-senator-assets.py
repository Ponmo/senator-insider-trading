...
def process_and_filter_records(records, target_year):
    """
...
def download_transactions(records, year):
    """
    ...
    for full_name, record in records.items():
        ...
        try:
            ...
            # Create CSV filename
            csv_filename = os.path.join("..", f"data/senator-assets-{year}", f"{full_name}.csv")
...
if __name__ == "__main__":
    print("Enter the year (2013-2024): ")
    year = input()
    if not year.isnumeric() or not 2013 <= int(year) <= 2024:
        print(f"{year} is not a valid year.")
        sys.exit()
    records = fetch_senate_financial_disclosures()
    filtered_records = process_and_filter_records(records, year)
    download_transactions(filtered_records, year)
